...
def preprocessData(dataframe):
    ...
    dataframe.drop(inplace=True, columns=['date',
                                          'round',
                                          'home_avg_corners',
                                          'away_avg_corners',
                                          'home_avg_fouls',
...
def predict(calc_dataframe,upcoming_dataframe):
    ...
    #columns=list(filter(lambda c: 'Unnamed' not in c ,set([k for k in upcoming_ds.element_spec.keys()]).union(predictipn_df.columns))) #clearing out unwanted columns
